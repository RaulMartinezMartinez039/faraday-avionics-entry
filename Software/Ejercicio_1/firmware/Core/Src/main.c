...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((uint32_t)(instante_actual - instante_anterior) >= 10U)
    {
      ...
      float tiempo_s = (float)(instante_actual % 120000U) / 1000.0f;
      ...
      if (tiempo_s <= 60.0f)
      {
        presion_prueba_pa = presion_inicio_pa
            + (presion_minima_pa - presion_inicio_pa) * tiempo_s / 60.0f;
      }
      else
      {
      ...
      float altura_nueva =
          AlturaDesdePresion(presion_prueba_pa, presion_inicio_pa);
